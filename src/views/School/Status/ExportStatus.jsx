...
const Export = ({ filteredRows, columnVisibility, COLUMNS, }) => {
  ...
  const ExportToPdf = (orientation) => {
    ...
    tableColumnHeaders.push("Pickup Status","Drop Status","Pickup Time", "Drop Time","Date","Request","Supervisor Name")
    ...
    filteredRows.forEach((row, index) => {
      ...
      COLUMNS().forEach((column) => {
       ...
       // Check if the value is a JSON object (like Attributes or Protocols)
      rowData.push(row.pickupStatus||"", row.dropStatus||"",row.pickupTime||"",row.dropTime||"",row.date||"",row.requestType||"",row.supervisorName||"")
  ...
  return (
    <div>
      <Button onClick={handleClick} variant="contained" color="success" aria-haspopup="true" aria-label="Export options"
      sx={{ padding: '6px 12px' }}
      >
        Export
      </Button>
