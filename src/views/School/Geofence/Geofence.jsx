    ...
    setFormData((prevData) => {
      ...
      if (name === 'area') {
        ...
        if (match) {
          ...
          const newLongitude =
            oldLongitude + radius / (111 * Math.cos((oldLatitude * Math.PI) / 180))
    ...
    try {
      ...
      const deleteRequests = selectedIds.map((_id) =>
        fetch(`${apiUrl}/${_id}`, {
          ...
          if (!response.ok) {
            throw new Error(`Error deleting record with ID ${_id}: ${response.statusText}`)
          }
