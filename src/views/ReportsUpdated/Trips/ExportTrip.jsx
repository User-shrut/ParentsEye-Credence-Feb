...
const Export = ({ filteredRows, columnVisibility, COLUMNS, pdfTitle, pdfFilename, excelFilename, }) => {
  ...
  const ExportToPdf = (orientation) => {
    ...
    COLUMNS().forEach((column) => {
      if (columnVisibility[column.accessor]) {
        ...
        tableColumnHeaders.push(column.accessor === "distance1" ? "Distance" : column.Header);
      }
    });
    ...
    filteredRows.forEach((row, index) => {
      ...
      COLUMNS().forEach((column) => {
        if (columnVisibility[column.accessor]) {
          ...
          if (column.accessor === "distance1") {
            value = (parseFloat(value) / 1000).toFixed(2) +" km" // Convert meters to km
          }
  ...
  return (
    <div>
      <Button onClick={handleClick} variant="contained" color="error" aria-haspopup="true" aria-label="Export options">
        Export
      </Button>
