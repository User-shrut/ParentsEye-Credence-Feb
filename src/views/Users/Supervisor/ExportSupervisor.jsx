...
const Export = ({ filteredRows, columnVisibility, COLUMNS, pdfTitle, pdfFilename, excelFilename, }) => {
  ...
  const ExportToPdf = (orientation) => {
    ...
    tableColumnHeaders.push("Actions"); // Add Actions header dynamically
    ...
    filteredRows.forEach((row, index) => {
      ...
      COLUMNS().forEach((column) => {
       ...
       // Check if the value is a JSON object (like Attributes or Protocols)
        // Add Actions column dynamically based on statusOfRegister
        rowData.push(row.statusOfRegister);
  ...
  return (
    <div>
      <Button onClick={handleClick} variant="contained"  color="primary" aria-haspopup="true" aria-label="Export options">
        Export
      </Button>
