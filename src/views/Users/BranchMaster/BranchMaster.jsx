...
const BranchMaster = () => {
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      ...
      const apiUrl =
        role == 1
          ? `${import.meta.env.VITE_SUPER_ADMIN_API}/delete-branch`
          : `${import.meta.env.VITE_SCHOOL_API}/delete-branch`
      const token = localStorage.getItem('token')
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      const apiUrl =
        role == 1
          ? `${import.meta.env.VITE_SUPER_ADMIN_API}/edit-branch/${selectedRow._id}`
          : role == 2
            ? `${import.meta.env.VITE_SCHOOL_API}/edit-branch/${selectedRow._id}`
            : null
  ...
  const handleAddSubmit = async () => {
    try {
      ...
      const apiUrl =
        role == 1
          ? `${import.meta.env.VITE_SUPER_ADMIN_API}/add-branch`
          : role == 2
            ? `${import.meta.env.VITE_SCHOOL_API}/add-branch`
            : null
  ...
  return (
    <>
      <h1 style={{ textAlign: 'center', marginTop: '20px' }}>Branch Master</h1>
      <div>
        <div
          style={{
            display: 'flex',
