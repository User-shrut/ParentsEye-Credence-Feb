...
const schooldriver = () => {
  ...
  const fetchData = async (startDate = '', endDate = '') => {
    ...
    try {
      ...
      let response
      if (role == 1) {
        response = await axios.get(`${import.meta.env.VITE_SUPER_ADMIN_API}/read-drivers`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
      } else if (role == 2) {
        response = await axios.get(`${import.meta.env.VITE_SCHOOL_API}/read-drivers`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
      } else if (role == 3) {
        response = await axios.get(`${import.meta.env.VITE_BRANCH_API}/read-drivers`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
      } else if (role == 4) {
        response = await axios.get(`${import.meta.env.VITE_USERBRANCH}/getdriverdata`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
          ...
          role == 1
            ? response?.data.data.flatMap((school) =>
                school.branches.flatMap((branch) =>
                  Array.isArray(branch.drivers) && branch.drivers.length > 0 ? branch.drivers : [],
                ),
              )
            : role == 2
        ...
        role == 1
          ...
          : role == 2
            ...
            : role == 3
              ...
              : `${import.meta.env.VITE_USERBRANCH}/deletedriverdata`
      const token = localStorage.getItem('token')
  ...
  const handleFileUpload = (event) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        axios
          .post(`${import.meta.env.VITE_API}/driver/import`, parsedData)
          .then((response) => {
        ...
        role == 1
          ...
          : role == 2
            ...
            : role == 3
              ...
              : `${import.meta.env.VITE_USERBRANCH}/updatedriverdata/${selectedRow.id}`
      const token = localStorage.getItem('token')
  ...
  const handleAddSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API}/driver/register`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleApprove = async (_id) => {
    try {
      ...
      if (role == 1) {
        ...
        response = await axios.post(
          `${import.meta.env.VITE_USERBRANCH}/approvedriver/${_id}`,
          { action: 'approve' },
          {
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  return (
    <>
      <h1 style={{ textAlign: 'center', marginTop: '20px' }}>Driver Detail List</h1>
      <div>
        <div
          style={{
            display: 'flex',
        ...
        >
          ...
          <Export
            ...
            pdfTitle={'DRIVER APPROVE'}
            pdfFilename={'DriverApprove.pdf'}
            excelFilename={'DriverApprove.xlsx'}
          />
        </div>
        <div
          style={{
            ...
            {COLUMNS()
              ...
              .map((col) => (
                <TextField
                  ...
                  fullWidth
                  InputProps={{
                    startAdornment: <InputAdornment position="start">{col.icon}</InputAdornment>,
                  }}
                />
              ))}
        ...
        <IconButton onClick={handleModalClose}>
          ...
          <TableBody>
            ...
            <p>
              ...
              are: Email (must be unique), Driver Name, Password{' '}
            </p>
            {/* File Upload Input */}
