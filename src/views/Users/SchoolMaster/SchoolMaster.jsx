...
const schoolmaster = () => {
      ...
      if (Array.isArray(response.data.schools)) {
        ...
        const mappedData = reversedData.map((row) => {
          const firstBranchName =
            row.branches && row.branches.length > 0 ? row.branches[0].branchName : 'No Branch' // Fallback if no branches are found
          return {
            ...row,
        ...
        setOriginalRows(
          allData.map((row) => {
            const firstBranchName =
              row.branches && row.branches.length > 0 ? row.branches[0].branchName : 'No Branch' // Fallback if no branches
  ...
  const handleDeleteSelected = async () => {
    try {
      ...
      const selectedIds = filteredRows.filter((row) => row.isSelected).map((row) => row._id)
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      const updatedRows = filteredRows.map((row) =>
        row._id === selectedRow._id ? { ...row, ...formData, isSelected: false } : row,
      )
  ...
  const handleAddSubmit = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_SUPER_ADMIN_API}/school-register`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
  ...
  const handleAccessClick = async (id, currentFullAccess) => {
    ...
    try {
      const response = await axios.put(
        `${import.meta.env.VITE_SUPER_ADMIN_API}/updateAccess/${id}`, // Pass row._id in the URL
        {
      ...
      <div>
        ...
        >
          ...
          <Export
            ...
            pdfTitle={'SCHOOL MASTER'}
            pdfFilename={'SchoolMaster.pdf'}
            excelFilename={'SchoolMaster.xlsx'}
          />
        </div>
        <div
          style={{
