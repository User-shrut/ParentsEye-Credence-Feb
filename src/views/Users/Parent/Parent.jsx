...
const Parent = () => {
  ...
  const fetchData = async (startDate = '', endDate = '') => {
    ...
    try {
      ...
      let response
      if (role == 1) {
        response = await axios.get(`${import.meta.env.VITE_SUPER_ADMIN_API}/read-parents`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
      } else if (role == 2) {
        response = await axios.get(`${import.meta.env.VITE_SCHOOL_API}/read-parents`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
      } else if (role == 3) {
        response = await axios.get(`${import.meta.env.VITE_BRANCH_API}/read-parents`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
          ...
          role == 1
            ...
            : role == 2
              ? response?.data.branches.flatMap((branch) =>
                  Array.isArray(branch.parents) && branch.parents.length > 0 ? branch.parents : [],
                )
              : role == 3
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      ...
      const apiUrl =
        role == 1
          ? `${import.meta.env.VITE_SUPER_ADMIN_API}/delete-parent`
          : role == 2
            ? `${import.meta.env.VITE_SCHOOL_API}/delete-parent`
            : role == 3
              ? `${import.meta.env.VITE_BRANCH_API}/delete-parent`
  ...
  const handleEditSubmit = async () => {
    ...
    const apiUrl =
      role == 1
        ? `${import.meta.env.VITE_SUPER_ADMIN_API}/update-parent`
        : role == 2
    ...
    try {
      ...
      if (role == 1) {
        response = await axios.post(
          `${import.meta.env.VITE_SUPER_ADMIN_API}/registerStatus/${_id}`,
          { action: 'approve' },
          {
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        response = await axios.post(
          `${import.meta.env.VITE_SCHOOL_API}/registerStatus/${_id}`,
          { action: 'approve' },
          {
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        response = await axios.post(
          `${import.meta.env.VITE_BRANCH_API}/registerStatus/${_id}`,
          { action: 'approve' },
          {
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  useEffect(() => {
    const fetchSchool = async (startDate = '', endDate = '') => {
      ...
      if (role == 1) {
        ...
        try {
          const token = localStorage.getItem('token')
          const response = await axios.get(`${import.meta.env.VITE_SCHOOL_API}/branches`, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
          })
        ...
        try {
          ...
          const branchname = response.data.data.flatMap((element1) =>
            Array.isArray(element1.branches) && element1.branches.length > 0
              ? element1.branches.map((branch) => ({ branchName: branch.branchName }))
              : [],
          )
  ...
  return (
    <>
      <h1 style={{ textAlign: 'center', marginTop: '20px' }}>Parent Detail List</h1>
      <div>
        <div
          style={{
            display: 'flex',
                        ...
                        >
                          ...
                          >
                            {row.statusOfRegister === 'pending' ? (
                              <>
                                <Button
                                  onClick={() => handleApprove(role == 3 ? row._id : row.parentId)}
                                  color="primary"
                                >
                                  Approve
                                </Button>
                                <Button
                                  onClick={() => handleReject(role == 3 ? row.id : row.parentId)}
                                  color="secondary"
                                >
                                  Reject
                                </Button>
          ...
          <Box sx={style}>
            ...
            <FormControl fullWidth sx={{ marginBottom: '10px' }}>
              <Autocomplete
                ...
                value={
                  filteredGeofences.find((geofence) => geofence.name === formData['pickupPoint']) ||
                  null
