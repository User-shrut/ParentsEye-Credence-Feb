...
export const AssignBranches = () => {
  ...
  const fetchData = async (startDate = '', endDate = '') => {
    ...
    try {
      ...
      let response
      if (role == 1) {
        response = await axios.get(`${import.meta.env.VITE_SUPER_ADMIN_API}/read-AssignBranchess`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
      } else if (role == 2) {
        response = await axios.get(`${import.meta.env.VITE_SCHOOL_API}/read-AssignBranchess`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
      } else if (role == 3) {
        response = await axios.get(`${import.meta.env.VITE_BRANCH_API}/read-AssignBranchess`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      ...
      const apiUrl =
        role == 1
          ? `${import.meta.env.VITE_SUPER_ADMIN_API}/delete-AssignBranches`
          : role == 2
            ? `${import.meta.env.VITE_SCHOOL_API}/delete-AssignBranches`
            : `${import.meta.env.VITE_BRANCH_API}/delete-AssignBranches`
      const token = localStorage.getItem('token')
  ...
  const handleFileUpload = (event) => {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        ...
        axios
          .post('http://63.142.251.13:4000/AssignBranches/import', parsedData)
          .then((response) => {
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      const apiUrl =
        role == 1
          ? `${import.meta.env.VITE_SUPER_ADMIN_API}/update-AssignBranches/${selectedRow.AssignBranchesId}`
          : role == 2
            ? `${import.meta.env.VITE_SCHOOL_API}/update-AssignBranches/${selectedRow.id}`
            : `${import.meta.env.VITE_BRANCH_API}/update-AssignBranches/${selectedRow.id}`
  ...
  const handleAddSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API}/AssignBranches/register`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    try {
      ...
      if (role == 1) {
        response = await axios.post(
          `${import.meta.env.VITE_SUPER_ADMIN_API}/registerStatus-AssignBranches/${_id}`,
          { action: 'approve' },
          {
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        response = await axios.post(
          `${import.meta.env.VITE_SCHOOL_API}/registerStatus-AssignBranches/${_id}`,
          { action: 'approve' },
          {
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        response = await axios.post(
          `${import.meta.env.VITE_BRANCH_API}/registerStatus-AssignBranches/${_id}`,
          { action: 'approve' },
          {
            headers: {
              Authorization: `Bearer ${token}`,
  ...
  return (
    <>
      <h1 style={{ textAlign: 'center', marginTop: '20px' }}>AssignBranches Detail List</h1>
      <div>
        <div
          style={{
            display: 'flex',
                          ...
                          >
                            {row.statusOfRegister === 'pending' ? (
                              <>
                                <Button
                                  onClick={() =>
                                    handleApprove(role == 1 ? row.AssignBranchesId : row.id)
                                  }
                                  color="primary"
                                >
                                  Approve
                                ...
                                <Button
                                  onClick={() =>
                                    handleReject(role == 1 ? row.AssignBranchesId : row.id)
                                  }
                                  color="secondary"
                                >
                                  Reject
          ...
          <Box sx={style}>
            ...
            {role == 1 && (
              <FormControl variant="outlined" sx={{ marginBottom: '10px' }} fullWidth>
                <Autocomplete
                  ...
                  value={
                    schools.find((school) => school.schoolName === formData['schoolName']) || null
            ...
            {(role == 1 || role == 2) && (
              <FormControl variant="outlined" sx={{ marginBottom: '10px' }} fullWidth>
                <Autocomplete
                  ...
                  value={
                    branches.find((branch) => branch.branchName === formData['branchName']) || null
            ...
            <FormControl variant="outlined" sx={{ marginBottom: '10px' }} fullWidth>
              <Autocomplete
                ...
                renderInput={(params) => (
                  <TextField {...params} label="Bus Name" variant="outlined" name="deviceId" />
                )}
              />
            </FormControl>
      ...
      <MenuItem key={option.deviceId} value={option.deviceId}>
        ...
        <IconButton onClick={handleModalClose}>
          ...
          <TableBody>
            ...
            <Box sx={{ overflowX: 'auto' }}>
              ...
              <Table size="small" sx={{ minWidth: 650 }}>
                <TableHead>
                  <TableRow>
                    {sampleData[0].map((header, index) => (
                      <TableCell key={index} sx={{ padding: '4px', fontSize: '0.85rem' }}>
                        {header}
                      </TableCell>
                    ))}
                  </TableRow>
                ...
                <TableBody>
                  {sampleData.slice(1).map((row, rowIndex) => (
                    <TableRow key={rowIndex}>
                      {row.map((cell, cellIndex) => (
                        <TableCell key={cellIndex} sx={{ padding: '4px', fontSize: '0.8rem' }}>
                          {cell}
                        </TableCell>
                      ))}
                    </TableRow>
            ...
            <p>
              Note: Please do not make changes to the column headers. Follow the sample Excel
              sheet's headings and fill in your data according to our format. The required fields
              are: Email (must be unique), AssignBranches Name, Password{' '}
            </p>
            {/* File Upload Input */}
