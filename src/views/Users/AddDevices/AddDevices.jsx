...
const AddDevices = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const firstApiResponse = await axios.get(`${import.meta.env.VITE_ROCKETSALES_API}/devices`, {
        headers: {
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      ...
      const apiUrl = `${import.meta.env.VITE_ROCKETSALES_API}/devices` // Replace with actual API endpoint
      const username = import.meta.env.VITE_USERNAME
      const password = import.meta.env.VITE_PASSWORD
  ...
  const handleEditSubmit = async () => {
    try {
      const apiUrl1 = `${import.meta.env.VITE_ROCKETSALES_API}/devices/${selectedRow.id}`
      const apiUrl2 = `${import.meta.env.VITE_SUPER_ADMIN_API}/add-device`
      ...
      if (response1.ok) {
        ...
        if (contentType && contentType.includes('application/json')) {
          ...
          if (response2.ok) {
            ...
            alert(
              `Unable to update record in the second API (POST): ${result2.message || response2.statusText}`,
            )
          }
        } else {
        ...
        alert(
          `Unable to update record in the first API: ${result1.message || response1.statusText}`,
        )
      }
      // Close the modal
      handleModalClose()
  ...
  useEffect(() => {
    const fetchGroups = async () => {
      ...
      try {
        ...
        const password = import.meta.env.VITE_PASSWORD
        const token = btoa(`${username}:${password}`)
        const response = await fetch(`${import.meta.env.VITE_ROCKETSALES_API}/groups`, {
          method: 'GET',
  ...
  useEffect(() => {
    const fetchCalendars = async () => {
      try {
        ...
        const password = import.meta.env.VITE_PASSWORD
        const token = btoa(`${username}:${password}`)
        const response = await fetch(`${import.meta.env.VITE_ROCKETSALES_API}/calendars`, {
          method: 'GET',
          headers: {
            Authorization: `Basic ${token}`,
          },
  ...
  const handleInputChange = (e) => {
    ...
    if (name === 'schoolName') {
      const selectedSchoolData = schools.find((school) => school.schoolName === value)
      ...
      if (selectedSchoolData) {
        ...
        if (selectedSchoolData.branches && selectedSchoolData.branches.length > 0) {
    ...
    const fetchSchool = async (startDate = '', endDate = '') => {
      ...
      try {
        ...
        const response = await axios.get(
          `${import.meta.env.REACT_APP_SUPER_ADMIN_API}/getschools`,
          {
            headers: {
  ...
  const handleAddSubmit = async () => {
    try {
      ...
      const apiUrl1 = `${import.meta.env.VITE_ROCKETSALES_API}/devices` // First API endpoint
      const apiUrl2 = `${import.meta.env.VITE_SUPER_ADMIN_API}/add-device` // Second API endpoint
      const username = import.meta.env.VITE_USERNAME
      const password = import.meta.env.VITE_PASSWORD
      ...
      if (response1.ok) {
        ...
        if (response2.ok) {
          ...
          console.error('Server responded with an error for the second API:', result2)
          alert(
            `Unable to create record in the second API: ${result2.message || response2.statusText}`,
          )
        }
      } else {
        console.error('Server responded with an error for the first API:', result1)
        alert(
          `Unable to create record in the first API: ${result1.message || response1.statusText}`,
        )
      }
    } catch (error) {
  ...
  return (
    <>
      <h1 style={{ textAlign: 'center', marginTop: '20px' }}>Add Devices </h1>
      <div>
        <div
          style={{
            display: 'flex',
