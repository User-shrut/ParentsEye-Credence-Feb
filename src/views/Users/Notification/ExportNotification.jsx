...
const Export = ({ filteredRows, columnVisibility, COLUMNS,}) => {
  ...
  const getNotifications = (row) => {
    const notificationMapping = {
      ...
      leaveRequestStatus: "Leave Request",
    };

  
    // Filter and map true values to their corresponding names
    return Object.keys(notificationMapping)
      .filter((key) => row[key])
      .map((key) => notificationMapping[key])
  ...
  const ExportToPdf = (orientation) => {
    ...
    tableColumnHeaders.push("Assign Notifications");
    ...
    filteredRows.forEach((row, index) => {
      ...
      COLUMNS().forEach((column) => {
       ...
       // Check if the value is a JSON object (like Attributes or Protocols)
      rowData.push(getNotifications(row));
  ...
  const ExportToExcel = async () => {
    ...
    const tableRows = filteredRows.map((row, index) => {
      ...
      COLUMNS().forEach((column) => {
       ...
       // Transform branches to a comma-separated string of branch names
       rowData.push(getNotifications(row));
      return rowData;
    });
    ...
    link.download = "Notifications.xlsx"; 
    link.click();
  };

  return (
    <div>
      <Button onClick={handleClick} variant="contained" aria-haspopup="true" aria-label="Export options">
        Export
      </Button>
