...
export const AppSidebarNav = ({ items }) => {
  ...
  // Filter items based on active section dashboard
  const filterItemsForSection6 = (section) => {
    console.log('Filtering for master dashboard:', section)
    if (section === 'school') {
      return items.filter((item) => item.name === 'School')
    }
    return items
  }

  // Filter items based on active section dashboard
  const filterItemsForSection7 = (section) => {
    console.log('Filtering for master dashboard:', section)
    if (section === 'users') {
      return items.filter((item) => item.name === 'Users')
    }
    return items
  }

  const filterItemsForSection8 = (section) => {
    console.log('Filtering for Expense management section:', section)
    if (section === 'support') {
  ...
  return (
    <>
      ...
      {school && (
        <CSidebarNav as={SimpleBar}>
          {filterItemsForSection6('school').map((item, index) =>
            item.items ? navGroup(item, index) : navItem(item, index),
          )}
        </CSidebarNav>
      )}
      ...
      {support && (
        <CSidebarNav as={SimpleBar}>
          {filterItemsForSection8('support').map((item, index) =>
            item.items ? navGroup(item, index) : navItem(item, index),
          )}
        </CSidebarNav>
      )}
